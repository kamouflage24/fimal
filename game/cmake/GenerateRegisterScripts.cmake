...
foreach(content_header_path IN LISTS header_paths)
    ...
    if (content_header_contents MATCHES "ScriptName[ \t\r\n]*=[ \t\r\n]*\"([^\"]+)\"")
        ...
        get_filename_component(content_header_dir "${content_header}" DIRECTORY)
        if (NOT content_header_dir STREQUAL "." AND NOT content_header_dir STREQUAL "")
            string(REPLACE "/" "::" content_namespace "${content_header_dir}")
            set(symbol_prefix "${content_namespace}::")
        else()
            set(namespace_matches "")
            string(REGEX MATCHALL "namespace[ \t\r\n]+([A-Za-z_][A-Za-z0-9_:]*)[ \t\r\n]*\\{" namespace_matches "${content_header_contents}")

            foreach(namespace_match IN LISTS namespace_matches)
                string(REGEX REPLACE "^namespace[ \t\r\n]+([A-Za-z_][A-Za-z0-9_:]*)[ \t\r\n]*\\{$" "\\1" namespace_name "${namespace_match}")
                if (NOT namespace_name STREQUAL "Canis")
                    set(symbol_prefix "${namespace_name}::")
                    break()
                endif()
            endforeach()
        endif()
